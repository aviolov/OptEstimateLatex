...
\abstract{Here we try to summarize the pieces deemed 'salvagable' from our
work on Optimal Design for SDEs based on maximizing the Mutual Information
between the process realizations and some prior belief distribution over the
parameters.}
...
\input{Figs/OU_MIControlSimulator/betasigmasweep_tabulate.txt}
...
% \section{The MI simplification}
% \label{sec:MI_simplification}
% 
% Let us give a rough explanation for what it means to go from the expression for
% the Mutual Information in \cref{eq:mutual_info_with_likelihood}. 
% $$
% I(\a) = \int_\Theta \int_X  \log\left[\frac{L(x|\th; \a)}
% 										{\int_\Theta L(x|\th;\a)\cdot \rho(\th) \intd{\th}} \right] \cdot L(x|
% 										\th;\a) \cdot \rho(\th) \intd{\th} \intd{x}
% $$
% 
% to the simpler expression in \cref{eq:mutual_info_time_integrated} 
% $$
% J(\a)  = \int_\th \int_0^T\int_X
%  \log\left(\frac{f(x,t|\th ; \a(\cdot) )}
%  			{\int_\th f(x,t|\th; \a(\cdot)) \rho(\th)\intd{\th}}\right) 
...
% $$
% 
% To explain the simplification, consider the 
% where the $dx$ integral has the same dimension as the dimension of the SDE not
% the dimension of the SDE times the number of observations. I should repeat that
% $J$ is NOT the mutual information between a realization of the process, $\{
% X_t\}_0^T$ and the parameter set, $\th$. It is the time-integral of the
